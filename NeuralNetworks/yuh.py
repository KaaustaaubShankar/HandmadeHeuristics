...
def train_test_split(x_data, y_data, split_percentage=0.8, shuffle_train=True,unique_digits = np.array([0,1,2,3,4])):
...
def update_params_with_momentum(network, gradients, velocities, alpha, beta):
    for i, (dW, db) in enumerate(gradients):
        # Skip updating the first layer
        if i == 0 or dW is None or db is None:
            continue
